...
NGS::Tools::GATK::Role::DepthOfCoverage

=head1 SYNOPSIS

A Perl Moose Role wrapper for the GATK DepthOfCoverage program.

=head1 ATTRIBUTES AND DELEGATES

=head1 SUBROUTINES/METHODS

=head2 $obj->generate_depth_of_coverage()

Generate the GATK DepthOfCoverage command.
...
=item * output: filename prefix for the output

=item * ref: Reference genome used for BAM alignment in FASTA format

=item * java: full path to the Java program

=item * gatk: full path to the GATK jar file (GenomeAnalysisTK.jar)
...
sub generate_depth_of_coverage {
	my $self = shift;
	...
	if ($args{'output'} eq '') {
		$output = join('.',
			File::Basename::basename($args{'bam'}, qw( .bam .sam )),
			'gatk',
			'depthofcoverage'
			);
		}
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc NGS::Tools::GATK::Role::DepthOfCoverage
...
1; # End of NGS::Tools::GATK::Role::DepthOfCoverage
